...
public class VendaDAO implements IDAO<Venda> {
    ...
    public void inserir(Venda venda) throws Exception {
        ...
        String sql = "INSERT INTO pedido (CODIGOCLIENTE, DATAVENDA, VALORTOTAL, SITUACAO) VALUES (?, ?, ?, ?)";
        ...
        for (ItemVenda iv : venda.getItens()) {
            sql = "INSERT INTO pedido (CODIGOPRODUTO, CODIGOVENDA, QUANTIDADE, VALORUNITARIO) VALUES (?, ?, ?, ?)";
    ...
    public void alterar(Venda venda) throws Exception {
        Conexao c = new Conexao();
        String sql = "UPDATE pedido SET CODIGOCLIENTE=?, DATAVENDA=?, VALORTOTAL=?, SITUACAO=? WHERE CODIGO=?";
    ...
    public ArrayList<Venda> listarTodos() throws Exception {
        ...
        String sql = "SELECT * FROM pedido ORDER BY DATAVENDA DESC";
        PreparedStatement ps = c.getConexao().prepareStatement(sql);
